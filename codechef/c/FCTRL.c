 ...
 * Station (BTS). These transceivers form the areas called cells 
 * (this term gave the name to the cellular phone) and every phone connects to 
 * the BTS with the strongest signal (in a little simplified view). Of course, 
 * BTSes need some attention and technicians need to check their function 
 * periodically.
 *
 ...
 *
 * For example, they defined the function Z. For any positive integer N, Z(N) 
 * is the number of zeros at the end of the decimal form of number N!. 
 * They noticed that this function never decreases. If we have two numbers 
 * N1<N2, then Z(N1) <= Z(N2). It is because we can never "lose" any
 ...
 * (equal to about 100000). It stands for the number of numbers to follow. Then 
...
int main(void)
{
  ...
  while(t--) {
    p = 0;
    scanf("%lld", &i);
    // Find and count multiples of 5
    // Some multiples such as 25, has 2 fives so we count two 5s
    for (j = 5; j <= i; j *= 5) {
      p = p + i / j;
    }
    printf("%d\n", p);
